...
A step runner supplies an instruction-file path for one already claimed article. Read that file,
...
skills or run scripts.

This session is the fallback for a record a constrained call could not answer or whose answer was
rejected, and the required lane for a record an audit sent back. Read the packet before deciding;
do not repeat an answer that was already refused.
